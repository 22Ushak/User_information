...
def filter_users():
    keyword = textbox_filter.get()
    if not keyword:
        messagebox.showinfo(title="Validation Error", message="Please enter a keyword to filter.")
        return

    try:
        with open(FILE_NAME, "r") as file:
            reader = csv.reader(file)
            rows = [row for row in reader if keyword.lower() in map(str.lower, row)]

            if len(rows) == 0:
                messagebox.showinfo(title="No Results", message="No matching users found.")
                return

            # Display filtered users in a new window
            filter_window = Toplevel(tk)
            filter_window.title("Filtered Users")
            filter_window.geometry("600x400")

            # Create treeview for displaying users
            tree = ttk.Treeview(filter_window, columns=("ID", "Name", "Address", "Designation"), show="headings")
            tree.heading("ID", text="ID")
            tree.heading("Name", text="Name")
            tree.heading("Address", text="Address")
            tree.heading("Designation", text="Designation")
        ...
        messagebox.showerror(title="Error", message=f"Error filtering users: {e}")
...
Label(text="Filter Users").grid(column=0, row=15, columnspan=2, pady=10)
Label(text="Keyword:").grid(column=0, row=16, sticky="E")
textbox_filter = Entry(width=30)
textbox_filter.grid(column=1, row=16)

Button(text="Filter Users", command=filter_users).grid(column=1, row=17, pady=10)
